...
> **Notion反映**: 正本（Notion優先）。§1-2〜1-8 深掘りはローカルのみ維持
...
```
顧客MTG（三原＋FDEチーム）→ 現場の困りごとを構造化 → プロダクト改修案
...
→ §2-1 の**テックリード**は「**部門の**AI環境をリードする役」。
...
※ CSE/DataOps は開発本体ではなく定例オペ中心 → §3-4 のとおり **CS配下維持**が筋が通る（方針は §2-2）。
...
**[要記入]** 「重視しないこと」（§2-1）— 例: 機能数だけの成功指標、EM依存の進捗管理 など
