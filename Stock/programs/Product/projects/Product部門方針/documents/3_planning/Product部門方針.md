...
> **最終同期**: 2026-05-25（ローカル → Notion 反映済）  
...
→ §2-2 の**テックリード**は「**部門の**AI環境をリードする役」（foroチームも利用）。
...
| 設計・スケジュール | チーム | 主体的に提案・実行・可視化 |
| 定例・差し込み | チーム | 定例は定期運用、差し込みは即計画化 |
| セキュリティ | チーム | セキュリティ担保、リスクの早期発見・対処 |
| 進捗・ブロッカー | チーム | デイリーで共有。マネージャーは障害除去 |
...
|------|---------|----------------|
| 顧客接点・案件立上 | FS／CS | |
| 顧客課題の深掘り・構造化・解決策 | FS＋CS＋FDE | 現場・業務の困りごとを共同で掘り下げ、解決策を見つける（構造化・優先度判断はFDEが担う） |
...
- [ ] FS／CSと共同で課題を深掘りできる
- [ ] 構造化・解決策・優先度判断まで担える
- [ ] AI支援下で設計〜実装を回せる（単独フルスタック完結は必須としない）
- [ ] 効果検証・改修まで含めて完遂できる
...
| リスク | 打ち手 |
|--------|--------|
| 顧客視点は強いが技術面が不安（スキル・稼働） | テックリード・馬場との連携（§2-3） |
| AI環境未整備 | テックリード(採用)・馬場・山田（§2-2-4） |
| 黒川の兼務（FDE / DataOps / AI DPC） | メインと兼務の整理（§2-4） |

**[要議論]**

- [ ] 最初のプロダクト範囲（入退院 only / AI DPC / 新規事業）
- [ ] 1人目（FDE採用）の時期と、foroチームとの人員境界
- [ ] AI環境の90日ゴール
- [ ] 成功指標: 現場課題→リリース件数、顧客フィードバック、リリース後効果
- [ ] テックリード採用の要否・要件（foroチームの技術面もカバーするか）

#### 2-2-4. FDEチームの期待役割

| 名前 | 担当 | 期待役割 |
|------|------|----------|
| 黒川 | FDE | FDEチームメイン<br>新規事業(AI DPC),DataOpsを兼務 |
| FDE(採用) | FDE | FDEのロールモデルの構築 |
| テックリード(採用) | FDE | 山田、馬場と協力してAI駆動開発の体制構築を推進する |
| データエンジニア(採用) | DataOps | DataOps担当 |
...
| 名前 | 担当領域 | 期待役割 |
|------|----------|----------|
| 三保谷 | foro | 業務委託へ資産整理、引き継ぎ |
| 馬場 | 全体 | FDE:AI駆動開発推進<br>foro:インフラ構成、セキュリティ |
| 山田 | 全体 | 部門全体のマネジメント、PdM |

**行動指針（未記入）**

- **重視しないこと**: （要記入）— 例: 機能数だけの成功指標、EM依存の進捗管理、探索と継続の一方への全振り
...
- FDEチームとforoチームどちらもメンバーが参画し、チームとして回っている状態
